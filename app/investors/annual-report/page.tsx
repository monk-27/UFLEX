
// "use client"

// import React from 'react'
// import Image from "next/image";
// import { SiteHeader } from '@/components/site-header';
// import { SiteFooter } from '@/components/site-footer';

// const page = () => {
//     return (
//         <div>
//             <SiteHeader />

//             <main className="bg-white">

//                 <section className="relative w-full h-[260px] sm:h-[320px] md:h-[380px] lg:h-[450px] overflow-hidden">

//                     <Image
//                         src="/images/investors/ar.png"
//                         alt="Investors Relations"
//                         fill
//                         className="object-cover w-full h-full"
//                         priority
//                     />

//                     <div className="absolute inset-0 bg-black/40"></div>

//                     <div className="absolute inset-0 flex flex-col justify-center px-6 sm:px-12 md:px-20 lg:px-28">
//                         <h1 className="text-white text-2xl sm:text-3xl md:text-[38px] lato-700 mb-4">
//                             Annual Report
//                         </h1>


//                     </div>
//                 </section>
//                 <div className="bg-[#EFEFEF]">
//                     <div className="max-w-7xl mx-auto pl-2 h-12 flex items-center">
//                         <span className="text-black text-xs lato-700 lato-600">
//                             Investors Relations &gt; Annual Report
//                         </span>
//                     </div>
//                 </div>


//             </main>
//             <SiteFooter />

//         </div>
//     )
// }

// export default page
"use client";

import React, { useState } from "react";
import Image from "next/image";
import Link from "next/link";
import { SiteHeader } from "@/components/site-header";
import { SiteFooter } from "@/components/site-footer";
import { quarterlyEarningsData } from "../ir-presentation-quarterly-earnings/page";
import Breadcrumb from "../breadcrumb";

/* ----------------------------- Types & data ----------------------------- */

type ArTabId = "uflex" | "subsidiaries" | "advertisement";

type ArTab = {
  id: ArTabId;
  label: string;
};

type DocItem = {
  text: string;
  link: string;
};

/* Top-level tabs */
const tabs: ArTab[] = [
  { id: "uflex", label: "UFLEX" },
  { id: "subsidiaries", label: "Subsidiary Companies" },
  { id: "advertisement", label: "Advertisement" },
];

/* ---------- UFLEX: Annual Reports list ---------- */
const annualReportsUFLEX: DocItem[] = [
  {
    text: "Annual Report for the Year 2024-2025",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2024-25.pdf",
  },
  {
    text: "Annual Report for the Year 2023-2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2023-24.pdf",
  },
  {
    text: "Annual Report for the Year 2022-2023",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2022-23.pdf",
  },
  {
    text: "Annual Report for the Year 2021-2022",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2021-22.pdf",
  },
  {
    text: "Annual Report for the Year 2020-2021",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2020-21.pdf",
  },
  {
    text: "Annual Report for the Year 2019-2020",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2019-20.pdf",
  },
  {
    text: "Annual Report for the Year 2018-2019",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2018-19.pdf",
  },
  {
    text: "Annual Report for the Year 2017-2018",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2017-18.pdf",
  },
  {
    text: "Annual Report for the Year 2016-2017",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AR-2016-17.pdf",
  },
  {
    text: "Annual Report for the Year 2015-2016",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AnnualReport-2016.pdf",
  },
  {
    text: "Annual Report for the Year 2014-2015",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEX-AnnualReport-2015.pdf",
  },
  {
    text: "Annual Report for the Year 2013-2014",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/UFLEXAnnualReport2014.pdf",
  },
  {
    text: "Annual Report for the Year 2012-2013",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Annual-Report-uflex2013.pdf",
  },
  {
    text: "Annual Report for the Year 2011-2012",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Annual-Report-uflex2012.pdf",
  },
  {
    text: "Annual Report for the Year 2010-2011",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Annual-Report-uflex2011.pdf",
  },
  {
    text: "Annual Report for the Year 2009-2010",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Annual-Report-uflex2010.pdf",
  },
  {
    text: "Annual Report for the Year 2008-2009",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Annual-Report-Uflex2009Deluxe.pdf",
  },
  {
    text: "Annual Report for the Year 2007-2008",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Annual-Report-Uflex-2008.pdf",
  },
  {
    text: "Annual Report for the Year 2006-2007",
    link: "https://beta.uflexltd.com/media/pdf/financials/annual-reports/Anuual-Report-uflex-2007.pdf",
  }

];

/* ---------- UFLEX: Quarterly Results – Consolidated ---------- */
const quarterlyConsolidated: DocItem[] = [
  {
    text: "Financial results for the Quarter ended 30.09.2025",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_RESULTS_30SEP2025.pdf",
  },
  {
    text: "Financial results for the Quarter ended 30.06.2025",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_RESULTS_30JUN2025.pdf",
  },
  {
    text:
      "Financial results for the Quarter and Year ended 31.03.2025 (Audited)",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_RESULTS_31MAR2025.pdf",
  },
  {
    text: "Financial results for the Quarter ended 31.12.2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_RESULTS_31DEC2024.pdf",
  },
  {
    text: "Financial results for the Quarter ended 30.09.2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_RESULTS_30SEP2024.pdf",

  },
  {
    text: "Financial results for the Quarter ended 30.06.2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_RESULTS_30JUN2023.pdf",
  },
  {
    text:
      "Financial results for the Quarter and Year ended 31.03.2024 (Audited)",
    link: "https://beta.uflexltd.com/media/pdf/financials/Consolidated/UFLEX_NSEBSE_CONS_AUDITED_RESULTS_31MAR2023.pdf",
  },
];

/* ---------- UFLEX: Quarterly Results – Standalone ---------- */
const quarterlyStandalone: DocItem[] = [
  {
    text: "Financial results for the Quarter ended 30.09.2025",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_QTRLY_RES_STA_30092025.pdf",
  },
  {
    text: "Financial results for the Quarter ended 30.06.2025",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_QTRLY_RES_STA_30062025.pdf",
  },
  {
    text:
      "Financial results for the Quarter and Year ended 31.03.2025 (Audited)",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_QTRLY_RES_STA_31032025.pdf",
  },
  {
    text: "Financial results for the Quarter ended 31.12.2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_QTRLY_RES_STA_31122024.pdf",
  },
  {
    text: "Financial results for the Quarter ended 30.09.2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_QTRLY_RES_STA_30092024.pdf",
  },
  {
    text: "Financial results for the Quarter ended 30.06.2024",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_QTRLY_RES_STA_30062024.pdf",
  },
  {
    text:
      "Financial results for the Quarter and Year ended 31.03.2024 (Audited)",
    link: "https://beta.uflexltd.com/media/pdf/financials/Standalone/UFLEX_NSEBSE_STA_AUDITED_RESULTS_31MAR2024.pdf",
  },
];

export type SubsidiaryItem = {
  title: string;
  link: string;
};

export const subsidiariesData: SubsidiaryItem[] = [
  {
    title: "UFlex Packaging Inc., USA",
    link: "https://www.uflexltd.com/",
  },
  {
    title: "Flex Films (USA) Inc., Kentucky, USA",
    link: "https://www.uflexltd.com/",
  },
  {
    title: "UFlex Europe Limited, UK",
    link: "https://www.uflexltd.com/",
  },
  {
    title: "Flex Middle East FZE, UAE",
    link: "https://www.uflexltd.com/",
  },
  {
    title: "Flex P. Films (Egypt) S.A.E., Egypt",
    link: "https://www.uflexltd.com/",
  },
  {
    title: "Flex Films Europa Sp.zo.o., Poland",
    link: "https://www.uflexltd.com/",
  },
];


// Advertisements (Financial Results) types
// ----------------- advertisements-data.ts -----------------

export type AdFYId =
  | "FY2025-2026"
  | "FY2024-2025"
  | "FY2023-2024"
  | "FY2022-2023"
  | "FY2021-2022"
  | "FY2020-2021"
  | "FY2019-2020";

export type AdDocument = {
  label: string;   // "Hindi", "English"
  href: string;    // PDF URL
};

export type AdQuarterCard = {
  id: string;          // unique id: "2025-09", etc.
  quarterLabel: string; // exact header text, e.g. "QUARTER ENDED - 30th September, 2025"
  docs: AdDocument[];
};

export type AdvertisementFY = {
  id: AdFYId;
  label: string;     // button text: "FY2025–2026"
  cards: AdQuarterCard[];
};

export const advertisementFYData: AdvertisementFY[] = [
  {
    id: "FY2025-2026",
    label: "FY2025–2026",
    cards: [
      {
        id: "2025-09",
        quarterLabel: "QUARTER ENDED - 30th September, 2025",
        docs: [
          { label: "Hindi", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2025-26/ADV-FR-30092025-JS.pdf" },
          { label: "English", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2025-26/ADV-FR-30092025-FE.pdf" },
        ],
      },
      {
        id: "2025-06",
        quarterLabel: "QUARTER ENDED - 30th June, 2025",
        docs: [
          { label: "Hindi", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2025-26/ADV-FR-30062025-JS.pdf" },
          { label: "English", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2025-26/ADV-FR-30062025-FE.pdf" },
        ],
      },
    ],
  },

  {
    id: "FY2024-2025",
    label: "FY2024–2025",
    cards: [
      {
        id: "2025-03",
        quarterLabel: "QUARTER ENDED - 31st March, 2025",
        docs: [
          { label: "Hindi", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-31032025-JS.pdf" },
          { label: "English", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-31032025-FE.pdf" },
        ],
      },
      {
        id: "2024-12",
        quarterLabel: "QUARTER ENDED - 31st December, 2024",
        docs: [
          { label: "Hindi", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-31122024-JS.pdf" },
          { label: "English", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-31122024-FE.pdf" },
        ],
      },
      {
        id: "2024-09",
        quarterLabel: "QUARTER ENDED - 30th September, 2024",
        docs: [
          { label: "Hindi", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-30092024-JS.pdf" },
          { label: "English", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-30092024-FE.pdf" },
        ],
      },
      {
        id: "2024-06",
        quarterLabel: "QUARTER ENDED - 30th June, 2024",
        docs: [
          { label: "Hindi", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-30062024-JS.pdf" },
          { label: "English", href: "https://beta.uflexltd.com/media/pdf/financials/advertisements/2024-25/ADV-FR-30062024-FE.pdf" },
        ],
      },
    ],
  },

  // Older years – you can fill when you have PDFs
  //   {
  //     id: "FY2023-2024",
  //     label: "FY2023–2024",
  //     cards: [],
  //   },
  //   {
  //     id: "FY2022-2023",
  //     label: "FY2022–2023",
  //     cards: [],
  //   },
  //   {
  //     id: "FY2021-2022",
  //     label: "FY2021–2022",
  //     cards: [],
  //   },
  //   {
  //     id: "FY2020-2021",
  //     label: "FY2020–2021",
  //     cards: [],
  //   },
  //   {
  //     id: "FY2019-2020",
  //     label: "FY2019–2020",
  //     cards: [],
  //   },
];


/* ----------------------------- Component ----------------------------- */

const Page = () => {
  const [activeTab, setActiveTab] = useState<ArTabId>("uflex");

  const isUFLEX = activeTab === "uflex";
  const isSubsidiary = activeTab === "subsidiaries";
  const isAdvertisement = activeTab === "advertisement";

  const renderRow = (item: DocItem, idx: number) => (
    <Link
      key={idx}
      href={item.link}
      target="_blank"
      className="group block"
    >
      <div
        className={[
          "w-full flex items-center justify-between px-6 py-2",
          "bg-[#F8F8F8] group-hover:bg-[#EDEDED]",
          "border-b border-white"
        ].join(" ")}
      >
        <span className="text-[14px] text-[#000000] lato-400">
          {item.text}
        </span>

        <Image
          src="/images/pdf.png"   // use your red icon here
          alt="PDF"
          width={20}
          height={20}
          className="shrink-0"
        />
      </div>
    </Link>
  );

  const renderSubsidiaryRow = (item: SubsidiaryItem, idx: number) => (
    <Link key={idx} href={item.link} target="_blank" className="group block">
      <div
        className={[
          "w-full flex items-center justify-between px-6 py-4",
          "bg-[#F8F8F8] group-hover:bg-[#EDEDED]",
          "border-b border-white",
        ].join(" ")}
      >
        <span className="text-[14px] text-[#000000] lato-400">

          {item.title}
        </span>

        <Image
          src="/images/pdf.png"
          alt="PDF"
          width={20}
          height={20}
          className="shrink-0"
        />
      </div>
    </Link>
  );
  const QuarterCard: React.FC<{ card: AdvertisementFY["cards"][number] }> = ({
    card,
  }) => {
    return (
      <div className="border border-[#E3E3E3] bg-[#FDFDFD]">
        {/* Red quarter header */}
        <div className="bg-gray-300 px-4 py-2">
          <h4 className="text-[#000000] text-[13px] sm:text-sm lato-700">
            {card.quarterLabel}
          </h4>
        </div>

        {/* Card body */}
        <div className="px-4 py-3 bg-white">
          <div className="text-[#000000] text-[13px] sm:text-sm lato-700 mb-2">
            Financial Results
          </div>
          <div className="h-[2px] bg-gray-300 mb-1" />

          <div className="mt-1">
            {card.docs.map((doc, idx) => (
              <Link key={idx} href={doc.href} target="_blank">
                <div className="flex items-center justify-between border-t border-dashed border-[#D95E52] first:border-t-0 py-2">
                  <span className="text-[13px] text-[#000000] lato-400">
                    {doc.label}
                  </span>
                  <Image
                    src="/images/pdf.png"
                    alt="PDF"
                    width={20}
                    height={20}
                    className="shrink-0"
                  />
                </div>
              </Link>
            ))}
          </div>
        </div>
      </div>
    );
  };

  const AdvertisementsSection: React.FC = () => {
    const [activeFY, setActiveFY] = useState<AdFYId>(
      advertisementFYData[0].id
    );

    const fy = advertisementFYData.find((f) => f.id === activeFY)!;

    return (
      <div className="mt-4">
        {/* FY pills row (blue filled / outline) */}
        <div className="flex flex-wrap gap-3 mb-5">
          {advertisementFYData.map((year) => {
            const active = year.id === activeFY;
            return (
              <button
                key={year.id}
                type="button"
                onClick={() => setActiveFY(year.id)}
                className={[
                  "px-4 py-2 rounded-full border text-xs sm:text-sm md:text-base lato-600 transition-all",
                  active
                    ? "bg-[#117ABA] text-white border-[#117ABA]"
                    : "bg-white text-[#000000] border-[#117ABA]",
                ].join(" ")}
              >
                {year.label}
              </button>
            );
          })}
        </div>

        {/* If no cards for that FY yet */}
        {fy.cards.length === 0 ? (
          <div className="border border-[#E3E3E3] bg-white px-4 py-8 text-center text-[13px] text-gray-500">
            Financial results advertisements for this financial year will be
            uploaded in due course.
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {fy.cards.map((card) => (
              <QuarterCard key={card.id} card={card} />
            ))}
          </div>
        )}
      </div>
    );
  };

  return (
    <div>
      <SiteHeader />

      <main className="bg-white">
        {/* HERO */}
        <section className="relative w-full h-[260px] sm:h-[320px] md:h-[380px] lg:h-[450px] overflow-hidden">
          <Image
            src="/images/investors/ar.png"
            alt="Investors Relations"
            fill
            className="object-cover w-full h-full"
            priority
          />

          <div className="absolute inset-0 bg-black/40" />

          <div className="max-w-7xl mx-auto absolute inset-0 flex flex-col justify-center px-6 sm:px-12 md:px-20 lg:px-28">
            <h1 className="text-white text-2xl sm:text-3xl md:text-[38px] lato-700 mb-4">
              Annual Report
            </h1>
          </div>
        </section>

        {/* BREADCRUMB */}
        {/* <div className="bg-[#EFEFEF]">
          <div className="max-w-7xl mx-auto pl-2 h-12 flex items-center">
            <span className="text-black text-xs lato-600">
              Investors Relations &gt; Annual Report
            </span>
          </div>
        </div> */}
        <Breadcrumb
          items={[
            { label: "Investors Relations", href: "/investors" },
            { label: "Annual Report" },
          ]}
        />

        {/* CONTENT */}
        <section className="max-w-6xl mx-auto px-4 sm:px-12 md:px-20 lg:px-28 pt-6 pb-10">
          {/* TOP TAB STRIP */}
          {/* <div className="flex w-fit border border-[#d6d6d6] rounded-sm overflow-hidden mb-4">
            {tabs.map((tab) => {
              const active = tab.id === activeTab;
              return (
                <button
                  key={tab.id}
                  type="button"
                  onClick={() => setActiveTab(tab.id)}
                  className={[
                    "relative px-6 py-2 text-sm lato-400 border-r border-[#d6d6d6] last:border-r-0",
                    active ? "bg-white text-black" : "bg-[#F5F5F5] text-[#555]",
                  ].join(" ")}
                >
                  {tab.label}
                  {active && (
                    <span className="absolute left-0 right-0 bottom-0 h-[3px] bg-[#117ABA]" />
                  )}
                </button>
              );
            })}
          </div> */}

          {/* UFLEX TAB CONTENT */}
          {isUFLEX && (
            <>
              {/* Annual Reports Header */}
              <div className="w-full bg-[#117ABA] py-3 px-4">
                <h2 className="text-white text-sm sm:text-base lato-700 text-center sm:text-center">
                  Annual Reports
                </h2>
              </div>

              {/* Annual Report list */}
              <div className="bg-white">
                {annualReportsUFLEX.map((item, idx) => renderRow(item, idx))}
              </div>

              {/* Consolidated Header */}
              {/* <div className="w-full bg-gray-300 text-[#000000] py-3 px-4">
                <h3 className="text-[#000000] text-sm sm:text-base lato-700 text-center">
                  Quarterly Results - Consolidated
                </h3>
              </div> */}

              {/* Consolidated list */}
              {/* <div className="bg-white">
                {quarterlyConsolidated.map((item, idx) => renderRow(item, idx))}
              </div> */}

              {/* Standalone Header */}
              {/* <div className="w-full  bg-gray-300 py-3 px-4">
                <h3 className="text-[#000000] text-sm sm:text-base lato-700 text-center">
                  Quarterly Results - Standalone
                </h3>
              </div> */}

              {/* Standalone list */}
              {/* <div className="bg-white">
                {quarterlyStandalone.map((item, idx) => renderRow(item, idx))}
              </div> */}
            </>
          )}



          {/* Subsidiaries placeholder (you'll plug screenshots later) */}

          {isSubsidiary && (
            <div className="bg-white">

              <div className="w-full bg-[#117ABA] py-3 px-4">
                <h2 className="text-white text-sm sm:text-base lato-700">
                  Subsidiary Companies
                </h2>
              </div>

              {subsidiariesData.map((item, idx) => renderSubsidiaryRow(item, idx))}

            </div>
          )}


          {/* Advertisement placeholder */}
          {isAdvertisement && <AdvertisementsSection />}

        </section>
      </main>

      <SiteFooter />
    </div>
  );
};

export default Page;

